...
cutseq ../process/SRR23538291/SRR23538291.fastq \
        -t 20 \
        -A INLINE \
        -m 20 \
        --trim-polyA \
...
hisat-3n --index ../ncrna_ref/Homo_sapiens.GRCh38.ncrna.fa \
    ...
    --new-summary \
    -q \
    -U ../process/SRR23538291/SRR23538291.fastq_cut\
    -p 16 \
    --base-change C,T \
    --mp 8,2 \
...
hisat-3n \
	...
	-p 16 \
	--summary-file ../process/SRR23538291/map2genome.output.summary \
	--new-summary \
	-q \
	-U ../process/SRR23538291/SRR23538291.mRNA.fastq \
	--directional-mapping \
	--base-change C,T \
	--pen-noncansplice 20 \
	--mp 4,1 \
...
samtools sort -@ 16 \
	--write-index \
	-O BAM \
	-o ../process/SRR23538291/SRR23538291.mRNA.genome.mapped.sorted.bam \
	../process/SRR23538291/SRR23538291.mRNA.genome.mapped.bam
...
samtools view \
	-@ 20 \
	-F 3980 \
	-c ../process/SRR23538291/SRR23538291.mRNA.genome.mapped.sorted.bam \
	> ../process/SRR23538291/SRR23538291.mRNA.genome.mapped.sorted.bam.tsv

echo "FINISH SAMTOOLS VIEW. GET: mRNA.genome.mapped.sorted.bam.tsv"

java -server -Xms8G -Xmx40G -Xss100M -Djava.io.tmpdir=../process/SRR23538291 -jar ../UMICollapse/umicollapse.jar bam -t 2 -T 16 --data naive --merge avgqual --two-pass -i ../process/SRR23538291/SRR23538291.mRNA.genome.mapped.sorted.bam -o ../process/SRR23538291/SRR23538291.mRNA.genome.mapped.sorted.dedup.bam > ../process/SRR23538291/SRR23538291.mRNA.genome.mapped.sorted.dedup.log
...
samtools view -e "rlen<100000" \
	...
	| cut -f 1,2,3,5,7 \
	| gzip -c \
	> ../process/SRR23538291/SRR23538291_unfiltered_uniq.tsv.gz
...
samtools view -e "rlen<100000" \
	...
	| cut -f 1,2,3,5,7 \
	| gzip -c \
	> ../process/SRR23538291/SRR23538291_unfiltered_multi.tsv.gz

echo "FINISH SAMTOOL. Get: unfiltered_multi.tsv.gz"

samtools view -@ 8 -e "[XM] * 20 <= (qlen-sclen) && [Zf] <= 3 && 3 * [Zf] <= [Zf] + [Yf]" ../process/SRR23538291/SRR23538291.mRNA.genome.mapped.sorted.dedup.bam -O BAM -o ../process/SRR23538291/SRR23538291.mRNA.genome.mapped.sorted.dedup.filtered.bam

echo "FINISH SAMTOOL [XM] * 20 <=. Get: mapped.sorted.dedup.filtered.bam"

samtools view -e "rlen<100000" -h ../process/SRR23538291/SRR23538291.mRNA.genome.mapped.sorted.dedup.filtered.bam | hisat-3n-table -p 16 -u --alignments - --ref ../ref/Homo_sapiens.GRCh38.dna.primary_assembly.fa --output-name /dev/stdout --base-change C,T | cut -f 1,2,3,5,7 | gzip -c > ../process/SRR23538291/SRR23538291_filtered_uniq.tsv.gz
