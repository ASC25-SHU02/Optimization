...
if [ "$enable_timing" == "yes" ]; then
	echo "**Timing enabled. See logs in ./log/stage1_SRR23538291.sh.log**"
    log_file="./log/stage1_SRR23538291.sh.log"
    exec > >(tee -a $log_file)
    exec 2>&1
    start_time=$(date +%s)
else 
...
run_with_timing 'cutseq ../process/SRR23538291/SRR23538291.fastq \
        -t 20 \
        -A INLINE \
        -m 20 \
        --trim-polyA \
...
run_with_timing 'java -server -Xms8G -Xmx40G -Xss100M \
	...
	> ../process/SRR23538291/SRR23538291.mRNA.genome.mapped.sorted.dedup.log'
